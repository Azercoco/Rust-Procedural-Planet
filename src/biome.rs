...
pub fn generate_biome_map(seed: i32, size: usize) -> (Vec<Vec<(f64, f64, f64)>>, f64){
    ...
    for i in 0..level{
        ...
        for j in 0..size{
            biome_map[i][j] = (r/2.0 + q*r/2.0, g/2.0 + q*g/2.0, b/2.0 + q*b/2.0);
        }
    }
    ...
    for _k in 0..n_k{
        ...
        for i in level..size{
            for j in 0..size{
                ...
                let dist = ((x - i as f64)*(x - i as f64) + (y - j as f64)*(y - j as f64)).sqrt()/range;
                let alpha = (-dist*dist).exp();
                let beta = 1.0 - alpha;

                let (r1, g1, b1) = biome_map[i][j];

                biome_map[i][j] = (alpha*r + beta*r1, alpha*g + beta*g1, alpha*b + beta*b1);
